...
例如，某CPU支持AVX-512指令集，FMA Units为2，CPU单周期双精度浮点计算能力为：

> 2 x 2 x 512 / 64 =32



参看[Tips to Measure the Performance of Matrix Multiplication Using Intel® MKL](https://software.intel.com/content/www/cn/zh/develop/articles/a-simple-example-to-measure-the-performance-of-an-intel-mkl-function.html)中**Theoretical Peak GFLOPS of test machine**小节的描述。
...
其中`gcc`、`gcc-c++`等基础库是必须的。



可选用的编译工具和运行库环境：

- intel工具集[Intel® oneAPI](https://software.intel.com/content/www/us/en/develop/tools/oneapi/commercial-base-hpc.html)（Intel® oneAPI Base & HPC Toolkit delivers）
...
```shell
make arch=Linux_Intel64  #编译生成的文件在bin/Linux_Intel64下
```
...
- MP LINPACK  分布式内存版本，用于多颗CPU计算机（SMP machine）以及多节点集群测试
...
mp_linpack各个文件说明可查看其中的readme文件内容。
...
- problem sizes (matrix dimension N) 
  ...
  - Ns 行
    ...
    根据of problems规定设置相应数量的N的值（也就是每次测试的规模大小）。

    求解问题规模越大浮点处理性能越高，但规模越大，测试时占用内存也更大。在集群测试中，内存为所有测试的节点的总内存。

    

    个人观点：对下文所述Ns取值方式存疑，扩大问题规模是为了更大压力下测试cpu性能，在内存较小的计算机上，可能出现尚未发挥出cpu最极限性能（cpu还可以求解更大规模问题且得到更高的测试值）但内存已经耗尽的情况。但是在超大内存的计算机，可能内存充裕到cpu发挥出极限性能也仅仅使用可用内存的一小部分，该大内存情况下，如此大的问题规模值或许没有意义，随着该值不断增大，最终测试值不再增长（甚至下跌）。因此，以下取值方式在如今内存越来越大的场景中或许并不那么实用，或可**根据经验设置较小的初始值，根据测试情况逐渐增大。**

    

    由于HPL对双精度(D P)元素的NxN数组执行计算，并且每个双精度元素需要8字节（memory alignment in double，8Byte=64bit）大小（单精度为32bit，4字节），因此Ns的问题大小所消耗的内存为`8Ns^2`。

    因为一般要为其他进程预留部分内存，因此消耗内存一般控制在实际内存的80%-90%，例如取内存的85%，则`8Ns^2=RAM*0.85`（RAM指内存大小，单位被Kb），**比较理想的问题规模值应该是NB值的倍数**（并非绝对），因此对使Ns整除NB的值，再用该值乘以NB的值，最终得到一个正整数。

    可得出N取值公式：
    $$
    N=\sqrt[2]{RAM×0.85/8}|NB×NB
    $$
    

    因此。*实际操作中可根据具体情况分析，监测内存占用量，适当调整规模值。*

    例如内存256G，NB为192，`(256×1024^3/8)*0.85|192=Ns*Ns*192`  ，Ns为165696。
...
- process grid (P x Q) 
  ...
  of process grids 表示P行和Q行（一个纵列的PQ值）要使用几组网格，该行数字为多少，则P行和Q行就要各自设置相应数量的数值。
...
- memory alignment in double
  ...
  双精度（double）计算的内存对齐值，双精度浮点数使用64位存储，这里的值的单位为Byte，因此填写8即可（8Byte=64bit）。
