...
if command -v mise &>/dev/null; then
	export MISE_DATA_DIR="$dev_env_dir/mise"
	export MISE_CACHE_DIR="$MISE_DATA_DIR/cache"
	eval "$(mise activate zsh)"
	alias miseprune='mise prune' miseup='mise upgrade'
	export MISE_ENV_SHELL_EXPAND=true
fi
...
#---python
export PIP_INDEX_URL="https://mirrors.cloud.tencent.com/pypi/simple" #https://mirrors.cernet.edu.cn/pypi/web/simple
...
show_ip() {
	local dg rows
	if command -v ip &>/dev/null; then #iproute
		dg=$(ip r | awk '/default/{print $3; exit}')
		rows=$(awk '
			NR==FNR { if($0~/via/) { for(i=1;i<=NF;i++) if($i=="dev") gw[$(i+1)]=$3 }; next }
			!/^(lo|169\.254)/ && $3 { print $1": "(gw[$1]?gw[$1]:"...")"  <--  "$3 }
		' <(ip r) <(ip -4 -br a))
	elif command -v ifconfig &>/dev/null; then
		dg=$(route -n get default | awk '/gateway/{print $2}')
		rows=$(awk '
			NR==FNR { if($1=="default" && $2 ~ /^[0-9]+\./) gw[$NF]=$2; next }
			/^[a-z]/ { i=$1; sub(/:/,"",i) }
			$1=="inet" && $2!~/^(127\.|169\.254)/ {
				for(j=1;j<=NF;j++) if($j=="netmask") m=$(j+1)
				split(m,h,"x"); n=0
				for(k=1;k<=length(h[2]);k++){ c=substr(h[2],k,1); n+=c~/[89a-fA-F]/?4:c~/[4-7]/?3:c~/[23]/?2:c~/[01]/?1:0 }
				print i": "(gw[i]?gw[i]:"...")"  <--  "$2"/"n
			}
		' <(netstat -rn) <(ifconfig))
	fi
	echo -e "\e[1;32mdefault gateway:\e[0m \e[1;35m$dg\e[0m"
	echo -e "\e[38;5;214m$(echo "$rows" | column -t)\e[0m"
}
...
Darwin)
	...
	alias up='brew cu -ay --no-brew-update ; brew update -v ; brew upgrade -g ; miseup ; pkgclean'
...
alias gme='git merge' gfe='git fetch' gpush='git push' gpull='git pull' gst='git status'
alias gsw='git switch' gbd='git branch -D' gcb='git checkout -b' gco='git checkout' gclean='git clean -fd'
alias glog='git log --oneline --graph --decorate --all'
alias gau='git add -u' gcm='git commit -m'
alias greset='git reset --hard' gback1='greset HEAD~1'
...
nosync_git_icloud() {
	...
	find "$dirpath" -type d -name ".git" | while read -r sub_dirpath; do
		mv -v "$sub_dirpath" "${sub_dirpath}.nosync"
		ln -s "${sub_dirpath}.nosync" "$sub_dirpath"
	done
}

#---network---
alias ipv4='curl -4s ipinfo.io/ip'
alias ipv6='curl -6s ipinfo.io/ip'
alias myip='curl -4s ipinfo.io|grep -vE "[\{\}]" && echo && echo IPv6: $(ipv6 || echo not found)'
alias ping='ping -c 4'
...
#---中文古诗词---
fortune_gushici() {
	# 统一使用用户级数据目录,不依赖 fortune 安装位置(升级/清理不受影响)
	FORTUNE_DIR=~/.local/share/fortune
	...
	# 仓库不再提供 .dat 索引,用 make install 安装(自动用当前系统 strfile 生成索引)
	git clone --depth 1 https://github.com/levinit/fortune-zh-gushici.git /tmp/fortune-zh-gushici &&
		cd /tmp/fortune-zh-gushici && make install FORTUNE_DIR=$FORTUNE_DIR &&
		cd - && \rm -rf /tmp/fortune-zh-gushici
}
...
alias history_all='history -i 1' # 从第1条开始显示
...
# 补全初始化：如果 dump 文件 (.zcompdump) 新于 24 小时，跳过安全检查以提速
