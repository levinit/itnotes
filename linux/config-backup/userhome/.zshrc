...
(command -v python3 &>/dev/null && (test -d $dev_env_path/python || mkdir -p $dev_env_path/python && ln -s $dev_env_path/python ~/.virtualenvs) &)
...
if [[ $os == Darwin ]]; then
  ...
  [[ $arch == x86_64 ]] && homebrew_dir=/usr/local
...
if [[ $os == Darwin ]]; then
  ...
  [[ $(uname -m) == arm64 ]] && ZSH_PLUGINS_DIR=/opt/homebrew/share
