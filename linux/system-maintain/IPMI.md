...
#查看SEL（代表 System Event Log，即 系统事件日志）查看各种硬件告警
ipmitool sel list
#获取指定sel日志详细信息（地址为sel lsit列表第一列所示，需要额外补充0x）
ipmitool setl get 0x1a 

#Field Replaceable Unit，即现场可更换单元（内存条、硬盘、电源模块、风扇等）信息
ipmitool fru
```
...
```shell
ipmitool user list 1  #查看当前用户列表，注意最后是1
