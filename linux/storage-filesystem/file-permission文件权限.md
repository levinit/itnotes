...
- 第2-10位：不同所有者对该文件的权限
  ...
  每个所有者占有三个位置，分别是：读位、写位和执行位。

  如果三个位置都有权限则依次显示： `r`（读）、`w`（写）和`x`（执行），如果某个位置无权限则显示为`-`。

  

  **注意：这三组的执行位（即第4、7、10位）若被设置了特殊权限，字符会发生变化。**具体参看下文的 [特殊权限管理](#特殊权限管理) 小节。

  - 第2-4位：文件所属用户的权限（u）。*若带有 SUID 特殊权限，执行位会显示为 `s` 或 `S`。*
  - 第5-7位：文件所属用户组的权限（g）。*若带有 SGID 特殊权限，执行位会显示为 `s` 或 `S`。*
  - 第8-10位：其他用户的权限（o）。*若带有 SBIT 粘滞位特殊权限，执行位会显示为 `t` 或 `T`。*
  ...
  **权限类型**，即对文件所具有的基本权限：
  ...
  使用`stat -c %a <filename>`可以获取以数字表示的权限信息，例如`stat -c %a /etc/hosts `返回的信息是`644`。
...
- `-h`或`--no-dereference`  作用于符号链接本身而非其指向的目标（适用于 chown / chgrp）
...
- chmod
  ...
  **特殊执行权限：大写 `X`**
  ...
  > 与小写 `x`（无条件赋予执行权限）不同，大写 `X` 是**条件执行权限**。
  >
  > - **作用机制**：只有当目标是**目录**，或者文件**原本就已经拥有至少一个 `x` 权限**时，`+X` 才会添加执行权限。纯文本文件会被自动跳过。
  > - **实战场景**：在给目录及其内部文件递归开放读取权限时，应使用 `chmod -R a+rX <目录>` 而不是 `a+rx`。这样可以保证目录能被进入，且不会把普通文本文件意外变成可执行文件。
  ...
  chmod g=rwx /srv
  chmod 775 /srv
  chmod -R a+rX /srv      #给目录及内部具有执行权限的文件递归加上读取和执行权限
...
* chown
  ...
  ```shell
  #chown [参数] <用户名>[:组名] <文件/目录> #组名也可以省略，以保持不变
...
使用`777`减去umask的值，即得到新建文件/目录的默认权限。

注意：新建文件总是不会有执行权限的，无论umask设置为何值，这是由操作系统（所有的Unix和类Unix系统）内核层面机制决定的。
...
执行位（即`ls -l`中看到的第一列的字符中的第4、7、10位）若被设置了特殊权限，该位字符发生变化：
...
使用 `ls -l` 查看特殊权限时，可能会出现大写的 `S` 或 `T`（例如 `-rwS------`）。

- 小写 `s` / `t`：正常生效。表示该位置原本**有**基础的执行权限（`x`）。
- 大写 `S` / `T`：无效/警告。表示该位置原本**没有**基础的执行权限（`-`），此时即使设置了特殊权限也无法触发。需要先补齐 `x` 权限。
...
以数字表示时，**在原来基本权限的3位数字前加上特殊权限的数字**，特殊权限同样遵循 `4(SUID) + 2(SGID) + 1(SBIT)` 的相加规则，示例：

```shell
chmod 4755 /tmp/test.sh
#相当于 chmod 755 /tmp/test.sh && chmod u+s /tmp/test.sh

chmod 6755 /tmp/test.sh
#6 = 4+2，同时设置 SUID 和 SGID

chmod 1777 /tmp
#1 = SBIT，所有人满权限并设置防删除粘滞位（经典如 /tmp 目录）
```
...
* user  用户： 对应`ACL_USER_OBJ`和`ACL_USER`
  ...
  > ACL_USER：定义了额外的用户可以对此文件拥有的permissi
...
**NFSv4 idmapd 报错解决**
报错 `name 'xxx' does not map into domain 'localdomain'` 是因为 NFSv4 强制使用 `user@domain` 的字符串形式来传递所有者信息。如果客户端和服务器的 Domain 不一致，或者无法在本地解析该用户名，就会回退为 `nobody` 或导致 invalid argument 报错。

**解决办法**：

1. 编辑服务端和客户端的 `/etc/idmapd.conf`。
2. 找到 `Domain = ...` 这一行，取消注释，并确保两边的 Domain 名称**完全一致**（例如都改为 `Domain = mycompany.local`）。
3. 重启映射服务：`systemctl restart nfs-idmapd` (或 `nfs-client.target`)。