...
tar添加以下参数指定压缩算法一次性实现归档tar包+压缩的组合操作：

- 自动选择压缩程序

  指定`-a`选项和带特定后缀的输出文件，程序将根据给定的要输出的压缩文件的后缀自动选择算法。

  ```shell
  tar -acvf test.tar.gz test  #tar检测到gz后缀，将自动选择gzip压缩
  #简便写法
  tar -acvf test{.tar.gz,}
  ```
...
- 指定压缩程序
  ...
  使用tar解压以上方式生成的压缩包，无需指定以上参数（即使指定也会被忽略）即可完成解压缩+解开tar包。
  
  ```shell
  tar -xJvf test.tar.xz test        #解压xz后解包tar
  tar -cJvf tets.tar.xz test        #打包tar后压缩为xz格式
  tar -cvf --zstd test.tar.zst test
  #BSD风格的短选项无-，但不能把--zstd放在cvf后面，不建议使用cvf这种写法避免记忆混淆
