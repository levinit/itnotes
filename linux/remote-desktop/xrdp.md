...
[xorgxrdp](https://www.xrdp.org/)是xrdp项目的一部分，一般在Linux发行版中包名即为xorgxrdp，另外也可以通过为英特尔和AMD GPU安装xorgxrdp-glamor以及为Nvidia GPU安装xorgxrdp-nvidia来启用OpenGL和Vulkan图形加速。
...
- xinit配置
  ...
  #---设置默认的桌面环境，如果要使用某个桌面环境，取消对应行的注释即可
  session=gnome          #GNOME
  ...
  session=${1:-$session} #如果有参数传入，使用传入的参数作为要启动的桌面环境/会话程序

  case $session in
  gnome)
      export XDG_SESSION_TYPE=x11
      export GDK_BACKEND=x11
      session=gnome-session #gnome-session --session=gnome-classic
      ;;
  kde* | plasma*)
