...
例如moudles安装在`/usr/share/modules`，则：
...
modules自带的模块文件在`/usr/share/Modules/modulefiles`（不同发行版可能略有差异）；而一些软件安装后会自动添加modules模块文件到`/etc/modulefiles`，无需自行编写模块，例如openmpi，使用`module avi`可看到新增了模块。

如果想自行增加其他目录到`module avi`列表中，可以添加该目录到`$MODULEPATH` 中，例如（以sh语法为例）：

```shell
#打算添加/share/config/modules
export MODULEPATH=/share/config/modules:$MODULEPATH
```
