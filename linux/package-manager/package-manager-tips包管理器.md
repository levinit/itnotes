...
yum/dnf提供了protected机制，保护重要的软件包不被意外删除。

两种配置方案

注意：都不支持使用通配符

- `/etc/yum.conf`或`/etc/dnf.conf`配置文件

  在配置文件的`[main]`部分添加`protected_packages`行，示例：

  ```ini
  [main]
  #其余内容省略
  protected_packages=kernel redhat-release gnome gnome-shell nautilus
  ```



- drop-in配置方案：`/etc/yum/protected.d/*.conf`或`/etc/dnf/protected.d/*.conf`

  在目录中创建`.conf`结尾的文件，文件中每行一个包名。示例`etc/yum/proteted.d/importan.conf`
...
## yum/dnf versionlock插件

`yum-plugin-versionlock`插件，dnf使用`dnf-plugin-versionlock`插件，安装后用法相同，yum改为dnf即可。
...
注意：被锁定的包，将不会出现在yum search列表中，yum安装时也会被忽略而提示不存在。
