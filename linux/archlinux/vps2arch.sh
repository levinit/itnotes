...
download_and_extract_bootstrap() {
    ...
    download iso/latest/sha256sums.txt | fgrep "$cpu_type.tar.zst" >"sha256sums.txt"
    read -r sha256 filename <"sha256sums.txt"
    download "iso/latest/$filename" >"$filename"
    grep -v archlinux-bootstrap-x86_64.tar.zst sha256sums.txt | sha256sum -c || exit 1
    tar -xpf "$filename"
    rm -f "$filename"
...
configure_chroot() {
    ...
    # Update the archlinux-keyring in bootstrap
...
install_packages() {
    ...
    is_openvz || packages="$packages linux lvm2"
...
finalize() {
    ...
    #---comment by levin 2023.10.27
    # reflector -l 35 -p https --sort rate --save /etc/pacman.d/mirrorlist

    #---Add by levin 2023.10.27
    timeout 10 reflector -l 35 -p https --sort rate --save /etc/pacman.d/mirrorlist || echo "$mirrors" >>/etc/pacman.d/mirrorlist
