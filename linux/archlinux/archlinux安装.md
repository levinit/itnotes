...
可选。较大内存可以不启用，建议使用zram。

swap也可以在安装完成后再配置。

### swapfile

推荐使用文件swap而不是独立的swap分区。

创建完毕后在`/etc/fstab`中添加类似`/swap none swap defaults,nofail 0 0 `即可。
...
用于在 RAM 中创建压缩块设备（即带有实时磁盘压缩的 RAM 磁盘）作为swap。
...
多系统os-prober需要编辑/etc/default/grub并添加该行（或取消该行注释）：
...
如果在生成引导命令执行后卡住，很久不能成功，参看下方[生成grub配置时挂起](#生成grub配置时挂起)。
