...
- 扩容qcow2
  ...
  2. **扩展分区表 (vda2)：** 使用 `growpart` 是最简单的。它会将 `vda2` 自动扩展到 `vda` 磁盘的末尾。

     Bash

     ```shell
     # 如果没有这个命令，安装 cloud-utils-growpart
     growpart /dev/vda 2
     ```
  ...
  4. 扩展逻辑卷 (LV) 并同步文件系统
     ...
     ```shell
     # 使用 -r 参数一次性完成 LV 扩展和文件系统同步（支持 XFS 和 Ext4）
     lvextend -l +100%FREE -r /dev/mapper/rlm_miwifi--rb06--srv-root
     ```
