...
2. 授权同步账户
   ...
   示例，在各个服务器均创建用户`sync`，密码`your_password`：
   
   ```mysql
   --允许访问的主机可以限定为指定的从服务器
   grant replication slave,file on *.* to 'sync'@'%' identified by 'your_password';
   flush privileges;
   --check
...
2. 在主服务器上导出数据库
   ...
   ```shell
   mysqldump --user=dbuser --password=your_password DB1 > db.sql
   #如果要到处函数和存储过程
   #mysqldump --user=dbuser --password=your_password -R -ndt DB1 > backup.sql
   ```
...
3. 将数据库文件存放到从服务器上，然后导入从服务器数据库：
   ...
   ```shell
   #创建数据库
   mysql --user=root --password=your_db_root_password -e " CREATE DATABASE DB1;" 
   #导入
   mysql --user=dbuser --password=your_password DB1 < backup.sql
   #检查
   mysql --user=dbuser --password=your_password -e "show databases" |grep DB1
   ```

4. 解锁主服务器数据库

   ⚠️ 注意：确保已经配置完主服务器并重启服务生效后（新数据库变更操作会记录到log_bin文件中），再解锁主服务器上已锁定的数据表：
...
2. 设定要同步的主服务器，开始同步
   ...
   change master to master_host='10.0.0.1', master_port=3306, master_user='sync',master_password='your_password';
...
架构中没有绝对的“主”或“从”角色，各节点既为主也为从，均能读写被同步的数据库。又称为主主复制（Master-Master）或双机/多机互备。
> 实际生产环境中也可演变为复杂的组合拓扑（如：两台机器互为主从，同时又各自挂载单向同步的只读从库）。

参照上文的主从复制操作：锁定并完全同步各个服务器数据库的初始状态后，**同时开启各自的 binlog 并将彼此添加为各自的主节点**。
...
参照[主从服务器同步初始状态](#主从服务器同步初始状态)，将各个服务器要同步的数据状态一致化。
...
**总结**：半同步复制是维持数据完整性和安全性的核心策略之一。虽然通过等待至少一个从节点收到日志的机制会产生一定的延迟（损失少许性能），但在生产环境的高可用场景下仍是必要的。其配置相对简单，关键在于理解其在性能与一致性间的取舍。
...
在双主互备架构下，如果同时在两个节点写入数据，可能发生主键（id）冲突。

可以在 `my.cnf` 中配置 `auto_increment_increment = 2` 和不同的 `auto_increment_offset`，让双主两台的 id 起始值与步长不同，来保障两台机器的 id 完全交错，这样在互相切换或同时写入过程中就不会导致 id 冲突。