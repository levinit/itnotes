...
2. 可选，执行以下命令进行初始化：
   ...
   MariaDB 安装后系统会创建 `mysql` 用户作为默认运行身份。初始化命令中的 `--user=mysql` 会将目录所有者改为该用户。如果未使用该参数配置，需手动修正存放目录权限：
...
一些 Linux 发行版中 MariaDB 默认只监听 `127.0.0.1`，在其配置文件中找到 `bind-address`，将值改为 `0.0.0.0` 即可允许任意 IP 连接。（提示：可在配置文件目录使用 `grep address * -r` 搜索）。
...
## 语句中含双引号时报错 Column not found

通常是因为 MySQL 配置的 `sql_mode` 中启用了 `ANSI_QUOTES`。

启用 `ANSI_QUOTES` 后，**不能用双引号 `""` 来引用字符串**，双引号包裹的文字会被解释为列名（标识符），从而提示找不到该列。此时必须使用单引号 `''` 来引用字符串。

**附：常见 `SQL_MODE` 选项**
- `STRICT_TRANS_TABLES`：严格模式（如果向事务表中插入非法值则直接中断操作）。
- `ALLOW_INVALID_DATES`：放宽日期校验（仅检查月/日数值范围，不完全校验实际合法性）。
- `ANSI_QUOTES`：将双引号 `""` 视为标识符符号（等同于反引号 \` \`），不可再用于字符串字面量。