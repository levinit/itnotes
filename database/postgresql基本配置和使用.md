...
配置文件位于初始化时指定的目录下的data文件夹中，常用的配置文件目录为`/var/lib/postgres/data/`：
...
- `pg_hba.conf`  数据库访问配置文件
  ...
  修改客户端登录验证，postgreSQL默认只允许本机连接，认证方式为ident。
  ...
  *提示：initdb方式初始化时若使用`-A`参数，则会自动为本地连接启动 "trust" 认证。*
  ...
  注意ADDRESS的值必须包含CIDR，例如不能写成127.0.0.1，可以写成127.0.0.1/32
  ...
  - 密码模式
    ...
    - `password`  未加密的口令
      ...
      口令是以明文形式在网络上发送的  ，建议只在在信任的网络中可以使用（127.0.0.1当然也可没有安全风险）。
  
    - `md5`  `scram-sha-256`  加密传输的的密码
...
>
> 角色可以拥有数据库和对象的权限，可以在数据库中创建、修改和删除对象，也可以授权给其他角色和用户。
>
> 用户是一个具体的实体，可以登录到 PostgreSQL 数据库并执行操作。用户可以是一个普通用户，也可以是一个超级用户（即具有所有权限的用户）。
>
> 在 PostgreSQL 中，每个用户都必须关联到一个角色，这个角色决定了用户的权限和访问级别。如果没有指定一个角色，那么 PostgreSQL 将为该用户创建一个默认角色。
>
> 因此，可以将角色看作是权限管理的实体，而用户则是实际执行操作的实体。通过将用户关联到角色，可以实现灵活的权限管理和访问控制。



根据角色自身的设置不同，一个角色可以是一个数据库用户或一组数据库用户。
