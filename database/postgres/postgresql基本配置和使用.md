...
如果使用systemd管理，使用reload：

```shell
systemctl reload postgresql
```

也可在psql中执行一下SQL命令进行配置重载：
...
- TYPE值：
  - `local`  使用 unix-domain socket
  - `host`、`hostssl`或`hostnossl` 使用 TCP/IP
  - `hostgssenc`或`hostnogssenc`

- ADDRESS值：
  - 留空表示本地网络
  - IP地址、主机名或CIDR（127.0.0.1/32），全部允许`0.0.0.0/0`
  - `samehost`或者`samenet`：允许当前服务器直连的任何同一子网内的主机连接
...
- METHOD值：
  ...
  - 以操作系统用户名为数据库用户名
    ...
    - `ident`  服务器鉴别认证
      ...
      通过联系客户端的 ident 服务器验证**操作系统用户名**是否匹配被请求的数据库用户名（只能在 TCP/IP 连接上使用）。
      **如果在本地连接指定了 `ident`，将自动被 `peer` 认证替代。**

    - `peer`  对等认证

      直接获取客户端的**操作系统用户名**作为数据库登录名进行校验（仅对本地 Unix/套接字连接可用）。
...
为避免混淆，下文提到的“用户”均特指 **数据库用户**，而非操作系统用户。

PostgreSQL 从 **8.1 版本开始，** **彻底统一了用户（User）和角色（Role）的概念**：
- **角色（Role）**：用于权限管理的抽象概念，类似“用户组”。
- **用户（User）**：本质上就是**带有登录（LOGIN）权限的角色**。

（注：PostgreSQL 安装后默认创建的超级用户就是 `postgres` 角色）。
...
- 创建新用户
  ...
  ```shell
  createuser <dbuser> -P  #创建一个名为dbuser的用户 (交互式创建可使用 --interactive)
  ```
...
- 数据库管理
  ...
  - 清理和分析数据库
    ...
    - vacuumdb
      ...
      它是 SQL 语句 `VACUUM` 的命令行程序封装，二者功能完全相同。
