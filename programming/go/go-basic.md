...
- 派生类型
  ...
  - 指针（Pointer）
    ...
    `&` 取地址符号，返回内存地址；`*`指针运算符，返回变量的值（从地址取得存储的值）。
...
make初始化时，可指定map的容量，该参数不是必须的，但指定一个合适的容量预先申请内存有助于提升性能，避免在后续使用中频繁扩张浪费性能。获取cap容量使用len()*。
...
Go使用channel通信来共享内存，实现并发的groutine之间（以及groutine和主线程之间）的连接，具体使用参看[并发编程](#并发编程)中channel使用的例子。
...
- 每个应用程序必须有切只有一个main package，执行时自动寻找其中的main()函数。

  应用程序指可生成可执行的程序，依赖包不需要main函数。



import -- go文件中引用其他go文件
...
协程：独立栈空间，共享堆空间，用户控制调度，一个线程上可以跑多个协程，协程是轻量级的线程。

go语言中协程实现为goroutine，一个goroutine的栈在其生命周期开始时只有很小的栈（典型情况下2KB），大幅减少的创建和销毁开销，是其高并发的根本原因。
...
在Go中，主线程并不会等待协程完成，在提交协程后就会返回主线程。这意味着如果主线程结束，所有未完成的协程也会被销毁，因此需要控制主线程等待协程的完成。
...
对于读文件，一般推荐使用buffio方式，小文件可使用`os.ReadFile()`。

- ReadFile/WriteFile 一次性读/写所有内容

  ```go
  content, err := os.ReadFile(file)   //读，返回所有内容数据类型是[]byte
  
  err := os.WriteFile(file, "string") //写，向指定文件写入字符串内容
  ```
...
如果在项目中为不同平台编写特定的go文件，可通过特定标识让编译器为不同平台进行编译。

如在 linux 下编译的文件，在文件名中添加`_linux`，在文件开头加上 `// +build linux` 即可。

示例，需要为hi.go编写三个平台linux、darwin和windows的代码，建立三个文件：

- `hi_linux.go`  ，首行添加`// +build linux`
- `hi_darwin.go`  ，首行添加`// +build darwin`  
- `hi_windows.go`  ，首行添加`// +build windows`
